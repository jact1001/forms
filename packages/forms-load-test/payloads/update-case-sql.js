...
export const useUpdateCasePayload = (caseId, name, formId, useCase) => {
    let newUseCase={
        "id": caseId,
        "case_name": name,
    ...
    for (let i = 0; i < useCase.sections.length; i++) {
        let section = useCase.sections[i];
        newUseCase.sections[i].id=section.id;
        for (let j = 0; j < section.fields.length; j++) {
            let field = section.fields[j];
            newUseCase.sections[i].fields[j].field_id=field.field_id;
            newUseCase.sections[i].fields[j].form_field_id=field.form_field_id;
        }
    }
